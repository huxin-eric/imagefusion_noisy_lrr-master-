...
image_data2 = im2double(sourceTestImage2); % 384*448
% image_data3
% wavelet level 2
wname = 'sym2';
N = 2;  % 小波分解级数
...
A2_x1 = appcoef2(C1,S1,wname,N);%level 2, low 二维离散小波变换低频部分系数提取
[H1_x1,V1_x1,D1_x1] = detcoef2('all',C1,S1,1);%level 1, high 二维离散小波变换高频部分系数提取
[H2_x1,V2_x1,D2_x1] = detcoef2('all',C1,S1,2);%level 2, high 二维离散小波变换高频部分系数提取
...
% low frequency
[w,h] = size(A2_x1);
lf_col1 = im2col(A2_x1, [unit, unit], 'distinct'); % 将矩阵A分为m×n的子矩阵，再将每个子矩阵作为B的一列
lf_col2 = im2col(A2_x2, [unit, unit], 'distinct');% 256*56
lf_colf = fusionLowCoe(lf_col1,lf_col2, type, unit);  % 低频系数融合
% type = 0
A2_xf = col2im(lf_colf,[unit, unit],[w,h],'distinct');  % 恢复图象的低频系数
% fusion_A2 = A2_xf;

% high frequency - first level
[w,h] = size(H1_x1);
hf1_col1 = im2col(H1_x1, [unit, unit], 'distinct');%‘distinct’表示将A分解为互不重叠的子矩阵，元素不足则补零
...
vf1_colf = fusionHighCoe(vf1_col1,vf1_col2,lambda, unit);  % 低频系数融合
...
df1_colf = fusionHighCoe(df1_col1,df1_col2,lambda, unit);  % 高频系数融合
